...
public class BoardController implements Initializable {
    ...
    public BoardController(SessionContext sessionContext, RequestDispatcher requestDispatcher, ViewNavigation viewNavigation, DependencyContainer dependencyContainer, InviteStudentFormController inviteStudentFormController) {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        showManagerView(sessionContext.hasRole(Role.MANAGER));
    ...
    private void showManagerView(boolean show){
        ...
        btnInvite.setManaged(show);
        btnAddTask.setVisible(show);
        btnAddTask.setManaged(show);
    }

    private void loadBoardCollaborators() {
        ...
        task.setOnSucceeded(event -> {
            for(CollaboratorView collab : task.getValue().collaborators()){
    ...
    void setOnClickRegisterStudent(Runnable onClickRegisterStudent) {
        this.onClickRegisterStudent = onClickRegisterStudent;
    }
    ...
    private void onDragDroppedStage(ListView<TaskView> source, DragEvent ev) {
        ...
        if (db.hasString()) {
            ...
            dev.builder.board.domain.model.Stage.StageState newStage = getStageFromListView(source);
    ...
    private dev.builder.board.domain.model.Stage.StageState getStageFromListView(ListView<TaskView> source) {
        if (source == todoList) {
            return dev.builder.board.domain.model.Stage.StageState.TO_DO;
        } else if (source == inProgressList) {
            return dev.builder.board.domain.model.Stage.StageState.IN_PROGRESS;
        } else {
            return dev.builder.board.domain.model.Stage.StageState.DONE;
        }
    }
    ...
    private void updateBoard() {
        Task<Optional<BoardView>> task = new Task<>() {
            ...
            protected Optional<BoardView> call() throws Exception {
                ...
                return sessionContext.hasRole(Role.MANAGER) ? requestDispatcher.dispatch(GetBoardQuery.own()) : requestDispatcher.dispatch(GetBoardQuery.forOwner(ownerEmail));
            }
        };
